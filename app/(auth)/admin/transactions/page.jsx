...
export default function AdminTransactionsPage() {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          proof_file_url,
          proof_file_name,
          payments ( id, payment_method, payment_status, paid_at )
        `)
      ...
      const mapped = data.map((item) => {
        ...
        let sessionType = item.consultation_type === "online" ? "Online Consultation" : "Offline Consultation";
        return {
          id: item.id,
          bookingCode: item.booking_code,
          user: item.client_name || "Guest",
          counselor: item.counselor_name || "Unknown Counselor",
          amount: item.price || 0,
          ...
          proofFileUrl: item.proof_file_url,
          proofFileName: item.proof_file_name,
          paymentId: payment.id,
        };
      });
      setTransactions(mapped);
      localStorage.setItem("adminTransactions", JSON.stringify(mapped));
    } catch (err) {
      console.error(err);
      const saved = JSON.parse(localStorage.getItem("adminTransactions")) || [];
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "payments" }, () => fetchTransactions())
      .subscribe();
  ...
  const handleApproveTransaction = async (tx) => {
    ...
    try {
      await supabase.from("payments").update({ payment_status: "success", paid_at: new Date().toISOString() }).eq("id", tx.paymentId);
      await supabase.from("consultations").update({ status: "confirmed" }).eq("id", tx.id);
      setActionMessage("Pembayaran berhasil diverifikasi.");
      fetchTransactions();
      if (selectedTransaction?.bookingCode === tx.bookingCode) {
        setSelectedTransaction((prev) => ({ ...prev, status: "Paid" }));
      }
    } catch (err) {
      console.error(err);
  ...
  const handleExportData = () => {
    const rows = [["Reference", "User", "Counselor", "Session Type", "Method", "Amount", "Date", "Status"]];
    filteredTransactions.forEach(tx => {
      rows.push([tx.reference, tx.user, tx.counselor, tx.sessionType, tx.method, tx.amount, tx.date, tx.status]);
    });
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#d9edf8]">
      ...
      <section className="relative z-10 w-full px-6 pb-6 pt-40 sm:px-8 lg:px-12">
        <div className="relative">
          {/* Header dan stats cards (sama seperti asli, tidak diubah) */}
          <div className="mb-7 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div><h1 className="text-[34px] font-bold leading-none text-[#e1268d] sm:text-[42px]">Transaction History</h1><p className="mt-2 text-[18px] text-[#f08bbf]">Track payments, revenue, and transaction status details</p></div>
            <div className="flex flex-col items-end gap-3"><div className="w-fit rounded-full bg-white px-5 py-2 text-[15px] font-medium text-[#e85fa7] shadow-sm">{formatTopDate(currentDate)}</div>{actionMessage && <div className="rounded-full bg-white/90 px-4 py-2 text-[13px] font-medium text-[#db2d8d] shadow-sm">{actionMessage}</div>}</div>
          </div>
          <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-[18px] bg-[#bde6e5]/85 px-5 py-5 shadow-[0_4px_12px_rgba(0,0,0,0.12)]"><p className="text-[14px] text-[#ea3f97]">Total Revenue</p><h3 className="mt-2 text-[28px] font-bold text-[#0c72a6]">{formatCurrency(totalRevenue)}</h3></div>
            <div className="rounded-[18px] bg-[#bde6e5]/85 px-5 py-5 shadow-[0_4px_12px_rgba(0,0,0,0.12)]"><p className="text-[14px] text-[#ea3f97]">Paid Transactions</p><h3 className="mt-2 text-[28px] font-bold text-[#0c72a6]">{paidTransactions}</h3></div>
            <div className="rounded-[18px] bg-[#bde6e5]/85 px-5 py-5 shadow-[0_4px_12px_rgba(0,0,0,0.12)]"><p className="text-[14px] text-[#ea3f97]">Pending Payments</p><h3 className="mt-2 text-[28px] font-bold text-[#0c72a6]">{pendingTransactions}</h3></div>
            <div className="rounded-[18px] bg-[#bde6e5]/85 px-5 py-5 shadow-[0_4px_12px_rgba(0,0,0,0.12)]"><p className="text-[14px] text-[#ea3f97]">Failed Payments</p><h3 className="mt-2 text-[28px] font-bold text-[#0c72a6]">{failedTransactions}</h3></div>
          </div>

          {/* Table transaction (sama seperti sebelumnya) */}
          <div className="mb-6 rounded-[20px] bg-white/85 p-5 shadow-[0_4px_12px_rgba(0,0,0,0.12)]">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div><h2 className="text-[18px] font-bold text-[#1e1e1e]">Transaction Directory</h2><p className="mt-1 text-[12px] text-[#5c5c5c]">Search and monitor all payment records</p></div>
      ...
      {/* MODAL VIEW TRANSACTION - DENGAN GAMBAR BUKTI */}
      {showViewModal && selectedTransaction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/25 px-4">
          <div className="w-full max-w-[550px] rounded-[24px] bg-white p-6 shadow-[0_10px_30px_rgba(0,0,0,0.18)] max-h-[90vh] overflow-y-auto">
            <div className="mb-5 flex items-start justify-between">
              <div><h2 className="text-[26px] font-bold text-[#db2d8d]">Transaction Detail</h2><p className="mt-1 text-[14px] text-[#777]">Detailed payment information</p></div>
              <button onClick={() => { setShowViewModal(false); setSelectedTransaction(null); }} className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f7f7f7] text-[18px] text-[#555]">×</button>
            </div>
            <div className="space-y-4">
              <div className="rounded-[14px] bg-[#fff5fa] px-4 py-3"><p className="text-[12px] text-[#ea3f97]">Reference ID</p><p className="mt-1 text-[15px] font-semibold text-[#222]">{selectedTransaction.reference}</p></div>
              <div className="rounded-[14px] bg-[#f4fbff] px-4 py-3"><p className="text-[12px] text-[#0c72a6]">User</p><p className="mt-1 text-[15px] font-semibold text-[#222]">{selectedTransaction.user}</p></div>
              <div className="rounded-[14px] bg-[#fff5fa] px-4 py-3"><p className="text-[12px] text-[#ea3f97]">Counselor</p><p className="mt-1 text-[15px] font-semibold text-[#222]">{selectedTransaction.counselor}</p></div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-[14px] bg-[#f4fbff] px-4 py-3"><p className="text-[12px] text-[#0c72a6]">Amount</p><p className="mt-1 text-[15px] font-semibold text-[#222]">{formatCurrency(selectedTransaction.amount)}</p></div>
                <div className="rounded-[14px] bg-[#fff5fa] px-4 py-3"><p className="text-[12px] text-[#ea3f97]">Date</p><p className="mt-1 text-[15px] font-semibold text-[#222]">{selectedTransaction.date}</p></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="rounded-[14px] bg-[#f4fbff] px-4 py-3"><p className="text-[12px] text-[#0c72a6]">Method</p><p className="mt-1 text-[15px] font-semibold text-[#222]">{selectedTransaction.method}</p></div>
              ...
              {selectedTransaction.proofFileUrl && (
                <div className="rounded-[14px] bg-[#fff0f7] p-4">
                  ...
                  <a href={selectedTransaction.proofFileUrl} target="_blank" rel="noopener noreferrer" className="block">
                    <img src={selectedTransaction.proofFileUrl} alt="Bukti Transfer" className="w-full max-h-64 object-contain rounded-xl border border-pink-200 cursor-pointer hover:opacity-90 transition" />
                  </a>
              ...
              <div className="flex justify-between pt-2">
                {selectedTransaction?.status === "Pending" && (
                  <button onClick={() => handleApproveTransaction(selectedTransaction)} className="rounded-full bg-[#2b9a45] px-5 py-2.5 text-[14px] font-medium text-white">Verify Payment</button>
                )}
                <button onClick={() => { setShowViewModal(false); setSelectedTransaction(null); }} className="rounded-full bg-[#db2d8d] px-5 py-2.5 text-[14px] font-medium text-white">Close</button>
              </div>
            </div>
          </div>
        </div>
