  ...
  return (
    <div className="min-h-screen bg-[#d4eefb] flex flex-col items-center justify-center py-10 px-4">
      ...
      <div className="relative w-[900px] mx-auto">
        ...
        <div className="absolute inset-0 flex px-8 py-6" style={{ left: "16px" }}>
          ...
          <div className="w-[68%] pr-4">
            <div className="bg-pink-500 text-white text-center py-1.5 rounded-t-lg font-semibold text-sm">Booking Confirmed!</div>
            <div className="bg-[#f3f3f3] p-3 rounded-b-lg space-y-3">
              <div className="flex gap-3">
                <div className="w-12 h-12 bg-gray-300 rounded-full flex-shrink-0" />
                <div className="flex-1">
                  <p className="font-bold text-xs leading-tight">{name}</p>
                  <p className="text-[10px] text-gray-600">Psikolog Klinis | Online</p>
                  <span className={`inline-block mt-1 px-2 py-0.5 text-[10px] rounded-full ${bookingStatusClass}`}>{bookingStatusLabel}</span>
              ...
              <div>
                ...
                <div className="h-10 bg-gray-200 rounded p-1 text-[9px] text-gray-600 overflow-y-auto">{topic || "No topic written."}</div>
              </div>
            </div>
          </div>
          {/* Kolom kanan - area barcode (dikosongkan) */}
      ...
      <div className="mt-10 text-center">
        ...
        <button
          onClick={() => { if (!canEnterChat) return; router.push(`/consultation/chat?bookingCode=${bookingCode}`); }}
          disabled={!canEnterChat}
          className={`mt-5 px-6 py-2 rounded-full font-semibold ${canEnterChat ? "bg-pink-300 text-pink-700" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
        >
        ...
        <button onClick={() => router.push("/consultation/my-bookings")} className="block mx-auto mt-3 px-6 py-2 bg-white text-[#0C72A6] rounded-full font-semibold">My Bookings</button>
      </div>
    </div>
  );
}