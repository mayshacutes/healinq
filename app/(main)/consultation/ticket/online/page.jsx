...
export default function TicketOnlinePage() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (diff <= 0) {
                ...
                setTimeLeft(status === "ongoing" ? "Session Ongoing" : "Session Ended");
                return;
            }
    ...
    const { counselorName: name, date, hour, bookingCode, topic, totalPayment, paymentMethod, paymentStatus, adminApproved, sessionDuration } = ticketData;
    const isPaymentVerified = ticketData.paymentStatus === "success";
    const bookingStatusLabel = ticketData.paymentStatus === "pending"
        ? "Pending Verification"
        : isPaymentVerified
        ? "Paid & Verified"
        : ticketData.paymentStatus || "Unknown";
    const bookingStatusClass = isPaymentVerified ? "bg-green-100 text-green-600" : paymentStatus === "Pending Verification" ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-700";
    ...
    return (
        <div className="min-h-screen bg-[#d4eefb] flex flex-col items-center justify-center py-10">
                    ...
                    <div className="w-[68%] pr-6">
                        ...
                        <div className="bg-[#f3f3f3] p-4 rounded-b-xl space-y-4">
                            <div className="flex gap-4">
                                ...
                                <div className="bg-pink-100 px-3 py-2 rounded-lg text-xs"><p className="font-semibold">Time Session</p><p>{hour} WIB</p></div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 bg-white p-4 rounded-2xl text-xs text-gray-700">
                                <div><p className="font-semibold">Session Type</p><p>Online Consultation</p></div>
            ...
            <div className="mt-12 text-center">
                ...
                <button onClick={() => { if (!canEnterChat) return; router.push(`/consultation/chat?bookingCode=${bookingCode}`); }} disabled={!canEnterChat} className={`mt-5 px-6 py-2 rounded-full font-semibold ${canEnterChat ? "bg-pink-300 text-pink-700 hover:opacity-90" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}>
                    {!isPaymentVerified ? "Waiting for Payment Verification" : sessionStatus === "upcoming" ? "Room Chat Not Started Yet" : sessionStatus === "finished" ? "Session Ended" : "Go to Room Chat"}
                </button>
