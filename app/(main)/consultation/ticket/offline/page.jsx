...
export default function TicketOfflinePage() {
  ...
  // ✅ useEffect 1: load data dari localStorage
  useEffect(() => {
  ...
  // ✅ useEffect 2: countdown timer (SATU-SATUNYA)
  useEffect(() => {
    if (!ticketData) {
      setTimeLeft("");
      return;
  ...
  // ✅ EARLY RETURN – semua hook sudah dipanggil di atas
  if (!ticketData) {
    return (
      <div className="min-h-screen bg-[#d4eefb] flex items-center justify-center p-10">
        <div className="bg-white rounded-2xl p-8 shadow text-center w-full max-w-lg">
          ...
          <p className="mt-3 text-gray-600">Data tiket tidak ditemukan. Pastikan booking sudah tersimpan dan coba lagi dari halaman My Bookings.</p>
        </div>
      </div>
    );
  }

  // Variabel dependen ticketData (boleh di sini)
    ...
    <div className="min-h-screen bg-[#d4eefb] flex flex-col items-center justify-center py-10">
          ...
          <div className="w-[68%] pr-6">
            ...
            <div className="bg-[#f3f3f3] p-4 rounded-b-xl space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 bg-white p-4 rounded-2xl text-xs text-gray-700">
                ...
                <div><p className="font-semibold">Payment</p><p>{ticketData.paymentMethod || "-"}</p></div>
                <div><p className="font-semibold">Status</p><p>{bookingStatusLabel}</p></div>
                <div><p className="font-semibold">Total</p><p>{formatRupiah(ticketData.totalPayment)}</p></div>
                <div><p className="font-semibold">Proof</p><p>{ticketData.proofFileName || "Not uploaded"}</p></div>
                <div><p className="font-semibold">Admin Verified</p><p>{ticketData.adminApproved ? "Yes" : "No"}</p></div>
              </div>
              <div>
                <p className="text-sm mb-1">☐ Consultation Preview</p>
                <div className="h-16 bg-gray-200 rounded-lg p-2 text-xs text-gray-600 overflow-hidden">{topic || "No topic written."}</div>
              </div>
            </div>
          </div>
          {/* RIGHT */}
      ...
      <div className="mt-12 text-center">
        ...
        <button
          ...
          className={`mt-5 px-6 py-2 rounded-full font-semibold ${attendanceConfirmed ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-pink-300 text-pink-700 hover:opacity-90"}`}
        >
