  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      const mapped = data.map((item) => ({
        ...
        paymentStatus: item.payments?.[0]?.payment_status || "pending",
      }));
      setBookings(mapped);
                ...
                <div key={booking.bookingCode} className="bg-white rounded-2xl p-6 shadow flex justify-between gap-6">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm text-gray-600">
                      ...
                      {booking.paymentStatus === "pending" && !verified && <p className="text-xs text-yellow-700 mt-1">Menunggu verifikasi bukti transfer.</p>}
